...
# Removes the sudoers-based read-only Docker rule from an older version of
# this script, if present. Dropped because Desktop Commander hard-blocks any
# command containing "sudo" in its own blockedCommands list (independent of
# OS permissions), so a sudo-based approach can never actually be used by the
# agent. Docker access is now granted via SupplementaryGroups=docker on the
# service unit instead (see the "9. Desktop Commander" section below).
remove_legacy_docker_readonly_sudo() {
    rm -f "/etc/sudoers.d/${DESKTOP_COMMANDER_USER}-docker"
}
...
if [ "$RUN_DESKTOP_COMMANDER" = "true" ]; then
    ...
    if getent group docker >/dev/null 2>&1; then
        # Full docker-group access for $DESKTOP_COMMANDER_USER - effectively
        # root-equivalent on this host. Chosen deliberately: a NOPASSWD sudoers
        # rule (narrower, read-only) was tried first but Desktop Commander
        # itself hard-blocks any command containing "sudo" (its own
        # blockedCommands list, independent of OS permissions), making that
