...
# Grants $DESKTOP_COMMANDER_USER read-only Docker visibility (docker ps, plus
# compose ps/logs for the files in DOCKER_READONLY_COMPOSE_FILES) via a narrow
# NOPASSWD sudoers rule. No exec/restart/up/down, no arbitrary containers - the
# docker group itself is root-equivalent, so this stays off unless configured.
configure_docker_readonly_sudo() {
    local sudoers_file="/etc/sudoers.d/${DESKTOP_COMMANDER_USER}-docker"
    local docker_bin
    docker_bin="$(command -v docker || true)"

    if [ -z "$docker_bin" ] || [ -z "$DOCKER_READONLY_COMPOSE_FILES" ]; then
        rm -f "$sudoers_file"
        print_info "Docker-Read-Zugriff für $DESKTOP_COMMANDER_USER übersprungen (Docker fehlt oder DOCKER_READONLY_COMPOSE_FILES ist leer)."
        return 0
    fi

    local compose_cmds="" f
    for f in $DOCKER_READONLY_COMPOSE_FILES; do
        if [ ! -f "$f" ]; then
            print_warning "DOCKER_READONLY_COMPOSE_FILES: Datei nicht gefunden, übersprungen: $f"
            continue
        fi
        compose_cmds="${compose_cmds}, ${docker_bin} compose -f ${f} ps, ${docker_bin} compose -f ${f} logs, ${docker_bin} compose -f ${f} logs --tail=100"
    done

    local tmp_sudoers
    tmp_sudoers="$(mktemp)"
    {
        echo "# Auto-generated by scripts/prodstart.sh - read-only Docker visibility for $DESKTOP_COMMANDER_USER."
        echo "# Scope: docker ps (host-wide, metadata only) and docker compose ps/logs, restricted to"
        echo "# the files listed in DOCKER_READONLY_COMPOSE_FILES (config.env). No exec, no restart,"
        echo "# no up/down, no arbitrary containers. Edit config.env and rerun prodstart to change"
        echo "# this - this file is regenerated on every run, manual edits are overwritten."
        echo "Cmnd_Alias DC_DOCKER = ${docker_bin} ps${compose_cmds}"
        echo "${DESKTOP_COMMANDER_USER} ALL=(root) NOPASSWD: DC_DOCKER"
    } > "$tmp_sudoers"

    if ! visudo -c -f "$tmp_sudoers" >/dev/null; then
        print_warning "Generierte sudoers-Regel für Docker-Read-Zugriff ist ungültig, wird nicht installiert."
        rm -f "$tmp_sudoers"
        return 1
    fi
    install -o root -g root -m 0440 "$tmp_sudoers" "$sudoers_file"
    rm -f "$tmp_sudoers"
    print_success "Docker-Read-Zugriff (ps + compose ps/logs) für $DESKTOP_COMMANDER_USER eingerichtet: $sudoers_file"
    return 0
}
...
prepare_desktop_commander() {
    ...
    if ! configure_docker_readonly_sudo; then
        print_warning "Docker-Read-Zugriff konnte nicht eingerichtet werden (nicht kritisch, wird übersprungen)."
    fi
...
DOCKER_READONLY_COMPOSE_FILES="${DOCKER_READONLY_COMPOSE_FILES:-}"
